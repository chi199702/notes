...
[pacman 命令详解](https://zhuanlan.zhihu.com/p/383694450) [如何优雅的使用 pacman](https://www.bilibili.com/video/BV1G4411e7iW/?spm_id_from=333.1007.top_right_bar_window_history.content.click)
...
1. 安装
   ...
   + S：同步安装软件包，与查询安装相关的命令都应该以该选项为前缀

     ```shell
     sudo pacman -S neovim
     ```

   + y：子选项，与远程数据库同步，通常和选项 u 一起使用

     ```shell
     sudo pacman -Sy
     ```
     
   + u：子选项，与远程数据库同步以后进行软件安装，常与选项 y 一起使用
   ...
   + yy：子选项，强制再次同步远程软件库
     ...
     ```shell
     # 由于不确定上次进行 sudo pacman -Sy 的时间，所以想强制再次进行同步
     sudo pacman -Syyu
     ```
...
3. 查询本地仓库
   ...
   + dt：不再需要的软件包（可能之前有软件（构建是）需要，后面卸载/升级后不再需要了）
